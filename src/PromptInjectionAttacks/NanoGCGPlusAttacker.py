...
class NanoGCGPlusAttacker(OptimizationAttacker):
    ...
    def gcg_search(self,args, clean_data, query,payload,position = "mid", target_answer=None):
        ...
        if "qwen" in self.model_name or "deepseek" in self.model_name or "secalign" in self.model_name or "prompt-guard" in self.model_name or "mistral" in self.model.name:
            no_improvement_threshold_prob =0
        else:
            no_improvement_threshold_prob = 0.0001
        for i_restart in range(self.n_restarts):    
            adv_init = get_adv_init(model_name = self.model_name)
            ...
            kv_cache,old_suffix_manager = initialize_kv_cache(model,context_left,context_right,payload,query, best_adv, target_answer)
            ...
            for it in range(1, n_iterations + 1):
                ...
                if not early_stopping_condition(best_logprobs, logprob_first_token, model,target_answer,no_improvement_threshold_prob=no_improvement_threshold_prob) and not it == n_iterations:  
                    ...
                    logprob,logprob_first_token = get_logprob_cache_test(kv_cache,old_suffix_manager,model,context_left,context_right,payload,query, adv, target_answer)
                    ...
                    logprob,logprob_first_token = get_logprob_cache_test(kv_cache,old_suffix_manager,model,context_left,context_right,payload,query, best_adv, target_answer)
                    if hasattr(model, 'guard_model'):
                        guard_logprob = model.get_guard_logprob(msg,payload)
                        if guard_logprob<-0.5:
                            logprob = (guard_logprob+logprob)
                ...
                if logprob > best_logprob:
                    ...
                    candidate_set = get_candidate_set_kv(kv_cache,model,context_left,context_right,payload,query, best_adv, target_answer)
                ...
                while True:
                    if count > 100:
                        print(f'Failed to find a valid candidate after 100 attempts')
